...
```{r}
Accuracy = fread('https://raw.githubusercontent.com/cszys888/BEGGER-DATA---Team-1/master/CloudFactory_DataSet_Accuracy_Prediction.tsv') #original data set
...
#Reformat the education and gender information of workers
...
colnames(test_detail2)[3] = "Task_Duration"
  ...
  #Create confision matrix lable and correspondent time cost for each task
    #Train Set
train_detail2 = train_detail2 %>% mutate(TN = ((train.accuracy=="TRUE")*(predict_train=="TRUE")),
...
train_detail2 = train_detail2 %>% select(-train.accuracy,-predict_train)
train_detail2 = train_detail2 %>% mutate(Cost_Benchmark = 3*Task_Duration*(TP+FN)+2*Task_Duration*(TN+FP),
    ...
    #Test Set
test_detail2 = test_detail2 %>% mutate(TN = ((test.accuracy=="TRUE")*(predict_test=="TRUE")),
...
test_detail2 = test_detail2 %>% select(-test.accuracy,-predict_test)
test_detail2 = test_detail2 %>% mutate(Cost_Benchmark = 3*Task_Duration*(TP+FN)+2*Task_Duration*(TN+FP),
                                         Cost_Omniscient = 2*Task_Duration*(TP+FN)+1*Task_Duration*(TN+FP),
                                         Cost_Model_ExcludingM = 2*Task_Duration*(TP+FP)+Task_Duration*(TN+FN))
